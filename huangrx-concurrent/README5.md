上篇从 ReentrantLock的角度，以AQS的独占锁，介绍了AbstractQueuedSynchronizer。
...
ReentrantLock 默认采用非公平锁，除非构造方法中传入参数 true。
...
### Condition
这块必须要先看懂上一篇关于 AbstractQueuedSynchronizer 的介绍，不然是看不懂的。

**注：只有AQS中next和prev的关系。Condition中是 Node中的 nextWaiter**

Condition 经常可以用在生产者-消费者的场景中，请看 Doug Lea 给出的这个例子

#### 例子
```java
package com.huangrx.concurrent;
...
我们常用 obj.wait()，obj.notify() 或 obj.notifyAll() 来实现相似的功能，但它们是 ==基于对象== 的监视器锁的。

而这里说的 Condition 是基于 ReentrantLock 实现的，而 ReentrantLock 是依赖于 AbstractQueuedSynchronizer 实现的。
...
![image-20221027095719787](https://images.huangrx.cn/uploads/2022/10/27/6359e58184039.png)
...
在addWaiter 方法中，有一个 unlinkCancelledWaiters() 方法，该方法用于清除队列中已经取消等待的节点。 当 await 的时候如果发生了取消操作（这点之后会说），或者是在节点入队的时候，发现最后一个节点是被取消的，会调用一次这个方法。
```java
...
savedState 作为 release 的参数，源代码中也就是 c = this.getState() - savedState，如果是同一个线程，会直接扣减为0，可重入可以进行释放
...
final boolean isOnSyncQueue(Node node) {
    ...
    if (node.waitStatus == Node.CONDITION || node.prev == null)
        return false;
    // 如果 node 已经有后继节点 next 的时候，那肯定是在阻塞队列了，只有AQS中next和prev的关系。
    if (node.next != null) 
        return true;
...
hread.interrupted()：如果当前线程已经处于中断状态，那么该方法返回 true，同时将中断状态重置为 false，所以，才有后续的 重新中断（REINTERRUPT） 的使用。
...
对于以上 3 种情况是最特殊的，因为他们能自动感知到中断（这里说自动，当然也是基于底层实现），并且在做出相应的操作后 **都会重置中断状态为 false**。
